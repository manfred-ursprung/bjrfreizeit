...
/**
 * Test case for class \Bjr\BjrLend\Domain\Model\Customer.
 *
 * @version $Id$
...
class CustomerTest extends \TYPO3\CMS\Extbase\Tests\Unit\BaseTestCase {
	/**
	 * @var \Bjr\BjrLend\Domain\Model\Customer
	 */
	protected $fixture;

	public function setUp() {
		$this->fixture = new \Bjr\BjrLend\Domain\Model\Customer();
	}
	...
	public function setOrdersForObjectStorageContainingOrderSetsOrders() { 
		...
		$objectStorageHoldingExactlyOneOrders = new \TYPO3\CMS\Extbase\Persistence\Generic\ObjectStorage();
		$objectStorageHoldingExactlyOneOrders->attach($order);
		$this->fixture->setOrders($objectStorageHoldingExactlyOneOrders);
	...
	public function addOrderToObjectStorageHoldingOrders() {
		$order = new \Bjr\BjrLend\Domain\Model\Order();
		$objectStorageHoldingExactlyOneOrder = new \TYPO3\CMS\Extbase\Persistence\Generic\ObjectStorage();
		$objectStorageHoldingExactlyOneOrder->attach($order);
		$this->fixture->addOrder($order);

		$this->assertEquals(
			$objectStorageHoldingExactlyOneOrder,
			$this->fixture->getOrders()
		);
	}
