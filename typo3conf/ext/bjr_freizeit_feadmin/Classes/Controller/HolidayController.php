...
use \MUM\BjrFreizeit\Domain\Repository\HolidayRepository;
 ...
 *
 */
class HolidayController extends \MUM\BjrFreizeitFeadmin\Controller\AbstractController {
    ...
    {
        $this->holidayRepository = $this->objectManager->get('MUM\\BjrFreizeit\\Domain\\Repository\\HolidayRepository');
	...
	public function listAction() {
		...
		$holiday = $this->holidayRepository->findAll();
        $js = $this->jsForDeleteHoliday();
        $GLOBALS['TSFE']->getPageRenderer()->addJsFooterInlineCode('holidayDelete', $js, false);

        $msg = $GLOBALS['TSFE']->fe_user->getKey("ses","holidayChange");
        if(!empty($msg)){
     ...
     * @return void
     */
    public function editAction(\MUM\BjrFreizeit\Domain\Model\Holiday $holiday) {
    ...
    /**
     * @param \MUM\BjrFreizeit\Domain\Model\Holiday $tag
     *
     */
    public function newAction(\MUM\BjrFreizeit\Domain\Model\Holiday $holiday = NULL){
     ...
     *
     */
    public function successUpdateAction(\MUM\BjrFreizeit\Domain\Model\Holiday $holiday){
    ...
    public function deleteAction() {
        $args = $this->request->getArguments();
        $holiday = $this->holidayRepository->findByUid($args['holiday']);
        $name = $holiday->getName();

        $this->holidayRepository->remove($holiday);
        $persistenceManager = $this->objectManager->get('TYPO3\\CMS\\Extbase\\Persistence\\Generic\\PersistenceManager');
        $persistenceManager->persistAll();
        $GLOBALS['TSFE']->fe_user->setKey("ses","holidayChange",'Die Ferienzeit '. $name .' ist gelöscht.');
        //return 'Ferienzeit ' . $name .' ist gelöscht';
        $this->redirect('list', 'Holiday');
    }


    protected function jsForDeleteHoliday(){
        $js = <<<EOT
